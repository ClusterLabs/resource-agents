...
/**
 * gfs2_logbh_init - Initialize a fake buffer head
 * @sdp: the filesystem
...
void
gfs2_logbh_init(struct gfs2_sbd *sdp, struct buffer_head *bh,
	       uint64_t blkno, char *data)
{
	ENTER(G2FN_LOGBH_INIT)
...
/**
 * gfs2_logbh_uninit - Clean up a fake buffer head
 * @sdp: the filesystem
...
void
gfs2_logbh_uninit(struct gfs2_sbd *sdp, struct buffer_head *bh)
{
	ENTER(G2FN_LOGBH_UNINIT)
	gfs2_assert_warn(sdp, test_bit(SDF_SHUTDOWN, &sdp->sd_flags) ||
			!buffer_busy(bh));
	gfs2_assert_warn(sdp, atomic_read(&bh->b_count) == 1);
	RET(G2FN_LOGBH_UNINIT);
}

/**
 * gfs2_logbh_start - Start writing a fake buffer head
 * @sdp: the filesystem
 * @bh: the buffer to write
 *
...
void
gfs2_logbh_start(struct gfs2_sbd *sdp, struct buffer_head *bh)
{
	ENTER(G2FN_LOGBH_START)
	submit_bh(WRITE, bh);
	RET(G2FN_LOGBH_START);
}

/**
 * gfs2_logbh_wait - Wait for the write of a fake buffer head to complete
 * @sdp: the filesystem
 * @bh: the buffer to write
 *
...
int
gfs2_logbh_wait(struct gfs2_sbd *sdp, struct buffer_head *bh)
{
	ENTER(G2FN_LOGBH_WAIT)

	wait_on_buffer(bh);

	if (!buffer_uptodate(bh) || buffer_dirty(bh)) {
		gfs2_io_error_bh(sdp, bh);
		RETURN(G2FN_LOGBH_WAIT, -EIO);
	}
	if (unlikely(test_bit(SDF_SHUTDOWN, &sdp->sd_flags)))
		RETURN(G2FN_LOGBH_WAIT, -EIO);

	RETURN(G2FN_LOGBH_WAIT, 0);
}
