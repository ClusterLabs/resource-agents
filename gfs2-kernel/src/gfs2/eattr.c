...
static int
ea_calc_size(struct gfs2_sbd *sdp,
	    struct gfs2_ea_request *er,
	    unsigned int *size)
{
	ENTER(G2FN_EA_CALC_SIZE)

	*size = GFS2_EAREQ_SIZE_STUFFED(er);
	if (*size <= sdp->sd_jbsize)
		RETURN(G2FN_EA_CALC_SIZE, TRUE);

	*size = GFS2_EAREQ_SIZE_UNSTUFFED(sdp, er);

	RETURN(G2FN_EA_CALC_SIZE, FALSE);
}
...
int
ea_check_size(struct gfs2_sbd *sdp, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_CHECK_SIZE)
	unsigned int size;

	if (er->er_data_len > GFS2_EA_MAX_DATA_LEN)
		RETURN(G2FN_EA_CHECK_SIZE, -ERANGE);
...
static int
ea_foreach_i(struct gfs2_inode *ip,
	     struct buffer_head *bh,
	     ea_call_t ea_call, void *data)
{
	ENTER(G2FN_EA_FOREACH_I)
	struct gfs2_ea_header *ea, *prev = NULL;
	int error = 0;

	if (gfs2_metatype_check(ip->i_sbd, bh, GFS2_METATYPE_EA))
		RETURN(G2FN_EA_FOREACH_I, -EIO);
	...
	for (ea = GFS2_EA_BH2FIRST(bh);; prev = ea, ea = GFS2_EA2NEXT(ea)) {
		...
		if (error)
			RETURN(G2FN_EA_FOREACH_I, error);
...
static int
ea_foreach(struct gfs2_inode *ip,
	   ea_call_t ea_call,
	   void *data)
{
	ENTER(G2FN_EA_FOREACH)
	struct buffer_head *bh;
	int error;
...
/**
 * gfs2_ea_find - find a matching eattr
 * @ip:
 * @er:
 * @el:
 *
...
int
gfs2_ea_find(struct gfs2_inode *ip,
	    struct gfs2_ea_request *er,
	    struct gfs2_ea_location *el)
{
	...
	if (error > 0)
		RETURN(G2FN_EA_FIND, 0);

	RETURN(G2FN_EA_FIND, error);
}
...
static int
ea_dealloc_unstuffed(struct gfs2_inode *ip,
		     struct buffer_head *bh,
		     struct gfs2_ea_header *ea,
		     struct gfs2_ea_header *prev,
		     void *private)
{
	ENTER(G2FN_EA_DEALLOC_UNSTUFFED)
	...
	if (GFS2_EA_IS_STUFFED(ea))
		RETURN(G2FN_EA_DEALLOC_UNSTUFFED, 0);
	...
	if (!rgd) {
		gfs2_consist_inode(ip);
		RETURN(G2FN_EA_DEALLOC_UNSTUFFED, -EIO);
	}
...
static int
ea_remove_unstuffed(struct gfs2_inode *ip,
		    struct buffer_head *bh,
		    struct gfs2_ea_header *ea,
		    struct gfs2_ea_header *prev,
		    int leave)
{
	ENTER(G2FN_EA_REMOVE_UNSTUFFED)
 ...
 out_alloc:
	...
	RETURN(G2FN_EA_REMOVE_UNSTUFFED, error);
}
...
{
	ENTER(G2FN_EA_REPACK)
	struct gfs2_holder gh;
	int error;

	error = gfs2_glock_nq_init(ip->i_gl, LM_ST_EXCLUSIVE, 0, &gh);
	if (error)
		RETURN(G2FN_EA_REPACK, error);
...
static int
ea_list_i(struct gfs2_inode *ip,
	  struct buffer_head *bh,
	  struct gfs2_ea_header *ea,
	  struct gfs2_ea_header *prev,
	  void *private)
{
	ENTER(G2FN_EA_LIST_I)
	struct ea_list *ei = (struct ea_list *)private;
	struct gfs2_ea_request *er = ei->ei_er;
	unsigned int ea_size = GFS2_EA_STRLEN(ea);

	if (ea->ea_type == GFS2_EATYPE_UNUSED)
		RETURN(G2FN_EA_LIST_I, 0);
...
int
gfs2_ea_list(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_LIST)
...
static int
ea_get_unstuffed(struct gfs2_inode *ip, struct gfs2_ea_header *ea,
		 char *data)
{
	ENTER(G2FN_EA_GET_UNSTUFFED)
	struct gfs2_sbd *sdp = ip->i_sbd;
	struct buffer_head **bh;
 ...
 out:
	...
	RETURN(G2FN_EA_GET_UNSTUFFED, error);
}
...
int
gfs2_ea_get_copy(struct gfs2_inode *ip,
		struct gfs2_ea_location *el,
		char *data)
{
	ENTER(G2FN_EA_GET_COPY)

	if (GFS2_EA_IS_STUFFED(el->el_ea)) {
		memcpy(data,
		       GFS2_EA2DATA(el->el_ea),
		       GFS2_EA_DATA_LEN(el->el_ea));
		RETURN(G2FN_EA_GET_COPY, 0);
	} else
		RETURN(G2FN_EA_GET_COPY,
...
int
gfs2_ea_get_i(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_GET_I)
	struct gfs2_ea_location el;
	int error;
	...
	if (!ip->i_di.di_eattr)
		RETURN(G2FN_EA_GET_I, -ENODATA);

	error = gfs2_ea_find(ip, er, &el);
	if (error)
		RETURN(G2FN_EA_GET_I, error);
	if (!el.el_ea)
		RETURN(G2FN_EA_GET_I, -ENODATA);
...
int
gfs2_ea_get(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_GET)
	struct gfs2_holder i_gh;
	int error;
...
static int
ea_alloc_blk(struct gfs2_inode *ip, struct buffer_head **bhp)
{
	ENTER(G2FN_EA_ALLOC_BLK)
	struct gfs2_sbd *sdp = ip->i_sbd;
	struct gfs2_ea_header *ea;
	uint64_t block;
	 ...
	 struct gfs2_ea_request *er)
{
	...
	if (GFS2_EAREQ_SIZE_STUFFED(er) <= sdp->sd_jbsize) {
		...
		for (x = 0; x < ea->ea_num_ptrs; x++) {
			...
			memcpy(bh->b_data + sizeof(struct gfs2_meta_header),
			       data, copy);
			if (copy < sdp->sd_jbsize)
				memset(bh->b_data + sizeof(struct gfs2_meta_header) + copy,
				       0, sdp->sd_jbsize - copy);
...
static int
ea_alloc_skeleton(struct gfs2_inode *ip, struct gfs2_ea_request *er,
		  unsigned int blks,
		  ea_skeleton_call_t skeleton_call, void *private)
{
	ENTER(G2FN_EA_ALLOC_SKELETON)
	struct gfs2_alloc *al;
	struct buffer_head *dibh;
	int error;
 ...
 out:
	...
	RETURN(G2FN_EA_ALLOC_SKELETON, error);
}
...
static int
ea_init_i(struct gfs2_inode *ip,
	  struct gfs2_ea_request *er,
	  void *private)
{
	ENTER(G2FN_EA_INIT_I)
	struct buffer_head *bh;
	int error;

	error = ea_alloc_blk(ip, &bh);
	if (error)
		RETURN(G2FN_EA_INIT_I, error);
...
static int
ea_init(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_INIT)
...
static struct gfs2_ea_header *
ea_split_ea(struct gfs2_ea_header *ea)
{
	ENTER(G2FN_EA_SPLIT_EA)
...
static void
ea_set_remove_stuffed(struct gfs2_inode *ip, struct gfs2_ea_location *el)
{
	ENTER(G2FN_EA_SET_REMOVE_STUFFED)
	...
	if (!prev || !GFS2_EA_IS_STUFFED(ea)) {
		ea->ea_type = GFS2_EATYPE_UNUSED;
		RET(G2FN_EA_SET_REMOVE_STUFFED);
...
static int
ea_set_simple_noalloc(struct gfs2_inode *ip,
		      struct buffer_head *bh,
		      struct gfs2_ea_header *ea,
		      struct ea_set *es)
{
	ENTER(G2FN_EA_SET_SIMPLE_NOALLOC)
	struct gfs2_ea_request *er = es->es_er;
	int error;

	error = gfs2_trans_begin(ip->i_sbd, RES_DINODE + 2 * RES_EATTR, 0);
	if (error)
		RETURN(G2FN_EA_SET_SIMPLE_NOALLOC, error);
...
static int
ea_set_simple_alloc(struct gfs2_inode *ip,
		    struct gfs2_ea_request *er,
		    void *private)
{
	ENTER(G2FN_EA_SET_SIMPLE_ALLOC)
	...
	if (error)
		RETURN(G2FN_EA_SET_SIMPLE_ALLOC, error);

	if (es->es_el)
		ea_set_remove_stuffed(ip, es->es_el);

	RETURN(G2FN_EA_SET_SIMPLE_ALLOC, 0);
}
...
static int
ea_set_simple(struct gfs2_inode *ip,
	      struct buffer_head *bh,
	      struct gfs2_ea_header *ea,
	      struct gfs2_ea_header *prev,
	      void *private)
{
	ENTER(G2FN_EA_SET_SIMPLE)
	struct ea_set *es = (struct ea_set *)private;
	unsigned int size;
	int stuffed;
	int error;
	...
	if (ea->ea_type == GFS2_EATYPE_UNUSED) {
		if (GFS2_EA_REC_LEN(ea) < size)
			RETURN(G2FN_EA_SET_SIMPLE, 0);
		if (!GFS2_EA_IS_STUFFED(ea)) {
			error = ea_remove_unstuffed(ip, bh, ea, prev, TRUE);
			if (error)
				RETURN(G2FN_EA_SET_SIMPLE, error);
		}
		es->ea_split = FALSE;
...
static int
ea_set_block(struct gfs2_inode *ip,
	     struct gfs2_ea_request *er,
	     void *private)
{
	ENTER(G2FN_EA_SET_BLOCK)
	struct gfs2_sbd *sdp = ip->i_sbd;
...
static int
ea_set_i(struct gfs2_inode *ip,
	 struct gfs2_ea_request *er,
	 struct gfs2_ea_location *el)
{
	ENTER(G2FN_EA_SET_I2)

	{
		...
		if (error > 0)
			RETURN(G2FN_EA_SET_I2, 0);
		if (error)
			RETURN(G2FN_EA_SET_I2, error);
	}
	{
		unsigned int blks = 2;
		if (!(ip->i_di.di_flags & GFS2_DIF_EA_INDIRECT))
...
static int
ea_set_remove_unstuffed(struct gfs2_inode *ip, struct gfs2_ea_location *el)
{
	ENTER(G2FN_EA_SET_REMOVE_UNSTUFFED)
	...
	RETURN(G2FN_EA_SET_REMOVE_UNSTUFFED,
...
int
gfs2_ea_set_i(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_SET_I)
	struct gfs2_ea_location el;
	int error;
	...
	if (!ip->i_di.di_eattr) {
		if (er->er_flags & XATTR_REPLACE)
			RETURN(G2FN_EA_SET_I, -ENODATA);
		RETURN(G2FN_EA_SET_I, ea_init(ip, er));
	}

	error = gfs2_ea_find(ip, er, &el);
	if (error)
		RETURN(G2FN_EA_SET_I, error);

	if (el.el_ea) {
		if (ip->i_di.di_flags & GFS2_DIF_APPENDONLY) {
			brelse(el.el_bh);
			RETURN(G2FN_EA_SET_I, -EPERM);
		}
...
int
gfs2_ea_set(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_SET)
	struct gfs2_holder i_gh;
	int error;
...
static int
ea_remove_stuffed(struct gfs2_inode *ip,
		  struct gfs2_ea_location *el)
{
	ENTER(G2FN_EA_REMOVE_STUFFED)
	struct gfs2_ea_header *ea = el->el_ea;
	struct gfs2_ea_header *prev = el->el_prev;
	int error;
	...
	if (error)
		RETURN(G2FN_EA_REMOVE_STUFFED, error);
...
int
gfs2_ea_remove_i(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_REMOVE_I)
	struct gfs2_ea_location el;
	int error;

	if (!ip->i_di.di_eattr)
		RETURN(G2FN_EA_REMOVE_I, -ENODATA);

	error = gfs2_ea_find(ip, er, &el);
	if (error)
		RETURN(G2FN_EA_REMOVE_I, error);
	if (!el.el_ea)
		RETURN(G2FN_EA_REMOVE_I, -ENODATA);
...
int
gfs2_ea_remove(struct gfs2_inode *ip, struct gfs2_ea_request *er)
{
	ENTER(G2FN_EA_REMOVE)
	struct gfs2_holder i_gh;
	int error;

	if (!er->er_name_len ||
	    er->er_name_len > GFS2_EA_MAX_NAME_LEN)
		RETURN(G2FN_EA_REMOVE, -EINVAL);

	error = gfs2_glock_nq_init(ip->i_gl, LM_ST_EXCLUSIVE, 0, &i_gh);
	if (error)
		RETURN(G2FN_EA_REMOVE, error);
...
ea_acl_chmod_unstuffed(struct gfs2_inode *ip,
		       ...
		       char *data)
{
	ENTER(G2FN_EA_ACL_CHMOD_UNSTUFFED)
	struct gfs2_sbd *sdp = ip->i_sbd;
	struct buffer_head **bh;
	unsigned int amount = GFS2_EA_DATA_LEN(ea);
	unsigned int nptrs = DIV_RU(amount, sdp->sd_jbsize);
	...
	bh = kmalloc(nptrs * sizeof(struct buffer_head *), GFP_KERNEL);
	if (!bh)
		RETURN(G2FN_EA_ACL_CHMOD_UNSTUFFED, -ENOMEM);
 ...
 out:
	...
	RETURN(G2FN_EA_ACL_CHMOD_UNSTUFFED, error);
...
int
gfs2_ea_acl_chmod(struct gfs2_inode *ip, struct gfs2_ea_location *el,
		 struct iattr *attr, char *data)
{
	ENTER(G2FN_EA_ACL_CHMOD)
	struct buffer_head *dibh;
	int error;
...
static int
ea_dealloc_indirect(struct gfs2_inode *ip)
{
	ENTER(G2FN_EA_DEALLOC_INDIRECT)
	struct gfs2_sbd *sdp = ip->i_sbd;
...
static int
ea_dealloc_block(struct gfs2_inode *ip)
{
	ENTER(G2FN_EA_DEALLOC_BLOCK)
	struct gfs2_sbd *sdp = ip->i_sbd;
	...
	if (!rgd) {
		gfs2_consist_inode(ip);
		RETURN(G2FN_EA_DEALLOC_BLOCK, -EIO);
	}
...
int
gfs2_ea_dealloc(struct gfs2_inode *ip)
{
	ENTER(G2FN_EA_DEALLOC)
	struct gfs2_alloc *al;
	int error;
...
int
gfs2_get_eattr_meta(struct gfs2_inode *ip, struct gfs2_user_buffer *ub)
{
	ENTER(G2FN_GET_EATTR_META)
	struct buffer_head *bh;
	int error;
