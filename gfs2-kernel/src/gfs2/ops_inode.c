	   ...
	   int mode, struct nameidata *nd)
{
	...
	if (!inode)
		RETURN(G2FN_CREATE, -ENOMEM);

	d_instantiate(dentry, inode);
	if (new)
		mark_inode_dirty(inode);

	RETURN(G2FN_CREATE, 0);
}
...
lookup_cdpn_sub_at(struct gfs2_sbd *sdp, struct dentry *dentry)
{
	...
	if (!buf)
		RETURN(G2FN_LOOKUP_CDPN_SUB_AT, ERR_PTR(-ENOMEM));

	parent = dget_parent(dentry);

	if (gfs2_filecmp(&dentry->d_name, "@hostname", 9))
	...
	RETURN(G2FN_LOOKUP_CDPN_SUB_AT, new);
}
...
lookup_cdpn_sub_brace(struct gfs2_sbd *sdp, struct dentry *dentry)
{
	...
	buf = kmalloc(2 * __NEW_UTS_LEN + 2, GFP_KERNEL);
	if (!buf)
		RETURN(G2FN_LOOKUP_CDPN_SUB_BRACE, ERR_PTR(-ENOMEM));

	parent = dget_parent(dentry);

	if (gfs2_filecmp(&dentry->d_name, "{hostname}", 10))
		new = lookup_one_len(system_utsname.nodename,
				     parent,
				     strlen(system_utsname.nodename));
	...
	RETURN(G2FN_LOOKUP_CDPN_SUB_BRACE, new);
}
...
gfs2_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	if (size > sdp->sd_sb.sb_bsize - sizeof(struct gfs2_dinode) - 1)
	        RETURN(G2FN_SYMLINK, -ENAMETOOLONG);
	...
	if (!inode)
		RETURN(G2FN_SYMLINK, -ENOMEM);

	d_instantiate(dentry, inode);
	mark_inode_dirty(inode);

	RETURN(G2FN_SYMLINK, 0);
}
...
gfs2_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	if (!gfs2_assert_withdraw(sdp, !error)) {
		...
		dent->de_type = cpu_to_gfs2_16(DT_DIR);
...
setattr_chown(struct inode *inode, struct iattr *attr)
{
 ...
 out_alloc:
	...
	RETURN(G2FN_SETATTR_CHOWN, error);
}
	     ...
	     int flags)
{
	...
	if (er.er_type == GFS2_EATYPE_UNUSED)
	        RETURN(G2FN_SETXATTR, -EOPNOTSUPP);
	     ...
	     void *data, size_t size)
{
	...
	er.er_type = gfs2_ea_name2type(name, &er.er_name);
	if (er.er_type == GFS2_EATYPE_UNUSED)
	        RETURN(G2FN_GETXATTR, -EOPNOTSUPP);
	er.er_data = data;
	er.er_name_len = strlen(er.er_name);
	er.er_data_len = size;
...
gfs2_removexattr(struct dentry *dentry, const char *name)
{
	...
	er.er_type = gfs2_ea_name2type(name, &er.er_name);
	if (er.er_type == GFS2_EATYPE_UNUSED)
	        RETURN(G2FN_REMOVEXATTR, -EOPNOTSUPP);
	er.er_name_len = strlen(er.er_name);
