      ...
      void *misc)
{
   ...
   if( gulm_info_stats_rpl == x_code ) {
      if( ccbp->status != NULL ) {
         if((err = ccbp->status(misc, lglcb_start, NULL, NULL)) != 0)
            goto exit;
      }
      do{
         if((err = xdr_dec_list_start(dec)) != 0 ) break;
         while( xdr_dec_list_stop(dec) != 0 ) {
            ...
            if((err = xdr_dec_string_ag(dec, &lg->cfbb, &lg->cfbb_len)) != 0)
               break;
            if( ccbp->status != NULL ) {
               if((err= ccbp->status(misc, lglcb_item, lg->cfba, lg->cfbb))!=0){
                  goto exit;
               }
            }
         }
      ...
      if( ccbp->status == NULL ) {err = 0; goto exit;}
 ...
 * Returns: int
 */
int lg_core_status(gulm_interface_p lgp)
{
   gulm_interface_t *lg = (gulm_interface_t *)lgp;
   xdr_enc_t *enc;
   int err;
