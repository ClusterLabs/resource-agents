...
typedef struct lg_lockspace_callbacks_s { 
  ...
  int (*logout_reply)(void *misc); 
  int (*lock_state)(void *misc, uint8_t *key, uint16_t keylen, uint8_t state, 
                    uint32_t flags, uint32_t error,  
                    uint8_t *LVB, uint16_t LVBlen); 
  int (*lock_action)(void *misc, uint8_t *key, uint16_t keylen, uint8_t action, 
                     uint32_t error); 
  int (*drop_lock_req)(void *misc, uint8_t *key, uint16_t keylen,uint8_t state);
