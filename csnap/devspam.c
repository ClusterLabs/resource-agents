...
void munge_block(char *buf, unsigned len, unsigned block, unsigned tag)
{
	struct spam {int pos; int block; int tag; char[4];} spam = (void *)buf;
	while (len > 0) {
		*spam++ = &(struct spam){i, block, tag, "SPAM"};
	}
}
