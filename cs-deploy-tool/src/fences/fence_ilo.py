...
def get_fence_name():
    return 'HP Integrated Light Out (iLO)'


class FenceIloSharedInfo(fence_base.FenceBaseSharedInfo):
    def __init__(self):
        fence_base.FenceBaseSharedInfo.__init__(self)
        pass
    

class FenceIlo(fence_base.FenceBase, gtk.HBox):

    def __init__(self, node, shared_info):
        fence_base.FenceBase.__init__(self, node, shared_info)
        gtk.HBox.__init__(self)
        
        self.hostname_entry = self.glade_xml.get_widget('fence_ilo_hostname')
        self.username_entry = self.glade_xml.get_widget('fence_ilo_username')
        self.password_entry = self.glade_xml.get_widget('fence_ilo_password')
        
        container = self.glade_xml.get_widget('fence_ilo')
        container.unparent()
    ...
    def get_nodes_fence_tag(self):
        template = '<device name=\"%s-ilo\"/>'
        hostname, username, password = self.__get_info()
        return template % self.node
    
    def get_fencedevice_tag(self):
        template = '<fencedevice agent=\"fence_ilo\" name=\"%s-ilo\" hostname=\"%s\" login=\"%s\" passwd=\"%s\"/>'
        hostname, username, password = self.__get_info()
