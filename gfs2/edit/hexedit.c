...
print_usage(void)
{
	...
	fprintf(stderr, "d\t\t- Display GFS2 structure\n");
	fprintf(stderr, "dx\t\t- Display GFS2 extended structure\n");
...
display()
{
	...
	for (row = 0; row < SCREEN_HEIGHT; row++) {
		...
		for (col = 0; col < SCREEN_WIDTH; col++)
			printf("%.2X ",
			       (unsigned char)buf[offset + SCREEN_WIDTH * row + col]);
		...
		for (col = 0; col < SCREEN_WIDTH; col++) {
			if (isprint(buf[offset + SCREEN_WIDTH * row + col]))
				printf("%c",
				       buf[offset + SCREEN_WIDTH * row + col]);
			else
				printf(" ");
		}
...
int
run_command(char *cmd, char *arg1, char *arg2, char *arg3)
{
	int error;
	...
	if (*cmd == 'b') {	/* Change block size */
	...
	else if (!strncmp(cmd, "dx", 2)) {	/* Display GFS2 structure */
		display_gfs2(TRUE);
	}

	else if (*cmd == 'd') {	/* Display GFS2 structure */
		display_gfs2(FALSE);
	}

	else if (*cmd == 'e') {	/* Edit GFS2 structure */
		edit_mode = !edit_mode;
	}
		...
		error = edit_gfs2(arg1, arg2, arg3);
