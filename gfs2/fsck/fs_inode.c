 ...
 * Returns: 0 on success, -1 on error
 */
int fs_copyin_dinode(struct fsck_inode *ip, struct buffer_head *dibh)
{
/*	struct buffer_head *dibh;*/
	...
	if(!dibh) {
		...
		if(check_meta(dibh, GFS2_METATYPE_DI)){
	...
	gfs2_dinode_in(&ip->i_di, BH_DATA(dibh));
...
static int make_dinode(struct fsck_inode *dip, struct gfs2_inum *inum,
                       unsigned int mode, osi_cred_t *cred)
{
	...
	{
		di.di_mode = mode | (S_ISDIR(mode) ? 02000 : 0);
	       ...
	       int *new, struct fsck_inode **ipp)
{
 ...
 retry:
	...
	error = fs_dir_add(dip, name, &inum, mode);
	if (error)
		goto fail;

	error = make_dinode(dip, &inum, mode, cred);
	if (error)
		goto fail;
...
int fs_mkdir(struct fsck_inode *dip, char *new_dir, int mode, struct fsck_inode **nip){
	...
	mode |= S_IFDIR;
	error = fs_createi(dip, &name, mode, &creds, &new, &ip);
	...
	ip->i_di.di_nlink = 2;
	ip->i_di.di_size = sdp->sb.sb_bsize - sizeof(struct gfs2_dinode);
	...
	di = (struct gfs2_dinode *)BH_DATA(dibh);
