...
static int check_leaf(struct fsck_inode *ip, uint64_t block, struct buffer_head **lbh,
		      void *private)
{
	uint64_t chain_no;
	struct fsck_sb *sbp = ip->i_sbd;
	struct gfs2_leaf leaf;
	struct buffer_head *chain_head = NULL;
		   ...
		   uint64_t parentblock)
{
	...
	if(!find_di(sbp, childblock, &di)) {
		if(di->dinode == childblock) {
			 ...
			 * it earlier */
			if(di->dotdot_parent && sbp->md.rooti->i_num.no_addr
			   != di->dinode) {
...
int check_dentry(struct fsck_inode *ip, struct gfs2_dirent *dent,
...
int build_rooti(struct fsck_sb *sbp)
{
	...
	create_inode(sbp, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH | S_IFDIR, &ip);
	block_set(sbp->bl, ip->i_di.di_num.no_addr, inode_dir);

	/* FIXME need to remove old 'root' entry from the master dir,
	 * and write a new one with this inode as the target */
	fs_dir_add(sbp->sb_master_dir, &(osi_filename_t){"root", 4},
		   &(ip->i_num), ip->i_di.di_mode);
	...
	/* Attach l+f to it */
	if(fs_mkdir(sbp->md.rooti, "l+f", 00700, &(sbp->lf_dip))){
...
/*		if(build_rooti(sbp)) {
  ...
  return -1;
	...
	if(!sbp->md.rooti || ds.q.block_type != inode_dir) {
		log_err("Invalid or missing root inode in"
			" superblock not\n", rootblock);
