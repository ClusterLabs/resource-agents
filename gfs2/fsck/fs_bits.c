			      ...
			      unsigned char new_state, int do_it)
{
	...
	goal = ((int)(goal - rgd->rd_ri.ri_data0) < 0)
		? 0
		: goal - rgd->rd_ri.ri_data0;


	block = fs_bitfit_core(sdp, goal, rgd->rd_ri.ri_data0,
	...
	switch(new_state) {
		/* FIXME: need to handle new states here -
		 * specifically DINODEs*/
	case GFS2_BLKST_FREE:
		block_set(sdp->bl, block + rgd->rd_ri.ri_data0, block_free);
		break;
	case GFS2_BLKST_USED:
		block_set(sdp->bl, block + rgd->rd_ri.ri_data0, block_used);
		break;
	case GFS2_BLKST_DINODE:
		/* FIXME: caller *must* set the bitmap appropriately
		 * after a succesful call */
		block_set(sdp->bl, block + rgd->rd_ri.ri_data0, meta_other);
		break;
	case GFS2_BLKST_INVALID:
		block_set(sdp->bl, block + rgd->rd_ri.ri_data0, meta_inval);
		break;
	default:
		return BFITNOENT;
	}
...
/*  struct fsck_rgrp	*rgd;*/
	...
	rgrp_block = (uint32_t)(blkno - rgd->rd_ri.ri_data0);
...
int fs_set_bitmap(struct fsck_sb *sdp, uint64_t blkno, int state){
	...
	fs_setbit(BH_DATA(rgd->rd_bh[buf]) + bits->bi_offset,
		  bits->bi_len,
		  (rgrp_block - (bits->bi_start*GFS2_NBBY)),
		  state);
